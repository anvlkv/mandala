...
use crate::{Angle, Float, Path, Point, Rect, Size, Transform, Vector};

pub fn rand_pt_in_bounds<R>(rng: &mut R, bounds: crate::BBox) -> crate::Point
where
    R: Rng,
{
    ...
    R: Rng + SeedableRng,
{
    ...
    pub transformations: Vec<GenTransform>,
}

#[derive(Debug, Clone)]
pub enum GenTransform {
    ...
    R: Rng + SeedableRng,
{
    ...
    pub fn generate(&mut self, gen_bounds: Rect) -> Vec<Path> {
        ...
        for (i, rect) in it {
            let path = render_fn(rng, rect.size);
            let mut t = Transform::translation(0.0, 0.0);

            for transofrm in self.transformations.iter() {
                match transofrm {
                    GenTransform::Scale(value) => {
                        let scale = value.value_at(i, rng);
                        t = t.then_scale(scale, scale);
                    }
                    GenTransform::Rotate(value) => {
                        let angle = value.value_at(i, rng);
                        t = t.then_rotate(angle);
                    }
                    GenTransform::Translate(value) => {
                        let by = value.value_at(i, rng);
                        t = t.then_translate(by);
                    }
                }
            }
            t = t.then_translate(Vector::new(rect.origin.x, rect.origin.y));
            result.push(path.transformed(t));
        }
        ...
        self.mode
            ...
            .into_iter()
            .map(|p| p.transformed(Transform::translation(by.x, by.y)))
            .collect()
    }
}
...
mod generator_tests {
    ...
    fn test_rand_pt_in_bounds() {
        ...
        let bounds = crate::BBox::new(Point::new(0.0, 0.0), Point::new(10.0, 10.0));
